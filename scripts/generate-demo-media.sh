...
for player in ace rin miko; do
  ...
  ffmpeg -hide_banner -loglevel warning -y \
    ...
    -f lavfi -i "anoisesrc=color=pink:amplitude=0.012:r=48000:d=828" \
    ...
    -filter_complex "[1:a]highpass=f=80,lowpass=f=4200[amb];[2:a]aformat=sample_rates=48000:channel_layouts=stereo,adelay=${miko_flash_ms}|${miko_flash_ms},volume=1.25[v1];[3:a]aformat=sample_rates=48000:channel_layouts=stereo,adelay=${rin_call_ms}|${rin_call_ms},volume=1.2[v2];[4:a]aformat=sample_rates=48000:channel_layouts=stereo,adelay=${miko_open_ms}|${miko_open_ms},volume=1.2[v3];[5:a]aformat=sample_rates=48000:channel_layouts=stereo,adelay=${laughter_ms}|${laughter_ms},volume=1.1[v4];[6:a]aformat=sample_rates=48000:channel_layouts=stereo,adelay=${celebration_ms}|${celebration_ms},volume=1.15[v5];[0:a][amb][v1][v2][v3][v4][v5]amix=inputs=7:duration=first:normalize=0,alimiter=limit=0.92[a]" \
    -map "[a]" -c:a aac -b:a 96k "$task_tmp/$player-audio.m4a"
  ...
  ffmpeg -hide_banner -loglevel warning -y \
    ...
    -c:a aac -b:a 96k -t 828 -movflags +faststart \
